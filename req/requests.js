...
app.get("/project_price_list_sort", (req, res) => {
    const rawQuery = db
        .query(
            `SELECT * FROM projects order by price ${req.query.price_sort}`,
            {
                type: QueryTypes.SELECT,
            }
        )
...
app.get("/project_client_point_list", (req, res) => {
    const rawQuery = db
        .query(`SELECT * FROM projects where client_id = ${req.query.client}`, {
            type: QueryTypes.SELECT,
        })
        .then((result) => {
            res.send(result);
...
app.get("/client_list_10", (req, res) => {
    const rawQuery = db
        .query("SELECT * FROM clients order by id limit 10", {
            type: QueryTypes.SELECT,
        })
        .then((result) => {
            res.send(result);
